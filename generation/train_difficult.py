...
# projectName = 'ICLR_2021_Workshop_MLPCP_Track_1_生成回复'

# corpus_paths = './data/t5_base/corpus.train.tfrecord'

#     './data/t5_base/corpus.train.tfrecord',
#     './data/t5_base/corpus.train.bert_entity.tfrecord', 

# 此预训练参数涉及隐私数据，还在沟通能否放出
init_checkpoint_path = './pretrain_weights/mt5_base/meddg_pretrain.h5'
config_path = './pretrain_weights/mt5_base/mt5_base_config.json'
spm_path = './pretrain_weights/mt5_base/sentencepiece_cn.model'
keep_tokens_path = './pretrain_weights/mt5_base/sentencepiece_cn_keep_tokens.json'
...
def make_dataset(corpus_path):
    ...
    def parse_function(serialized):
        ...
        features = {
            ...
            're_t_token_ids': tf.io.FixedLenFeature([max_out_len], tf.int64),
        ...
        return {
            ...
            'Decoder-Input-Token': features['re_t_token_ids'],
            'Decoder-Output-Token': features['t_token_ids'],
        }, {
            'mlm_loss': 0.0,
        }
...
def build_transformer_model_with_mlm():
    ...
    # 辅助输入
    decoder_output = Input(shape=(max_out_len,), dtype=tf.int64, name='Decoder-Output-Token')  # 目标id

    output = MLMLoss(name='mlm_loss')([decoder_output, model.outputs[0]])

    train_model = Model(model.inputs + [decoder_output], output)
